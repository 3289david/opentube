...
export default function ShortsPage() {
  ...
  // IntersectionObserver for infinite scroll (bottom sentinel)
  useEffect(() => {
  ...
  // Track active item by scroll position
  useEffect(() => {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/yt/api/trending?type=shorts&pageToken=${nextPageToken}`)
      const data = await res.json()
    ...
    <div className="relative h-full bg-black overflow-hidden">
      ...
      >
        {shorts.map((short, idx) => (
          ...
          >
            {/* YouTube iframe (portrait) */}
            <div className="relative h-full max-h-full flex items-center justify-center w-full">
              <div className="relative w-full h-full max-w-sm mx-auto">
                {idx === activeIndex ? (
                  ...
                  <div className="w-full h-full relative">
                    <Image src={short.thumbnail} alt={short.title} fill className="object-cover" sizes="400px" />
              ...
              <div className="absolute right-4 bottom-24 flex flex-col items-center gap-5 z-20">
                ...
                <button onClick={() => toggleLike(short.id)} className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-white text-xs font-medium">{liked[short.id] ? '좋아요' : '좋아요'}</span>
                </button>

                {/* Comment */}
                <Link href={`/yt/watch/${short.id}`} className="flex flex-col items-center gap-1">
                ...
                <button onClick={() => setMuted(m => !m)} className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-white text-xs font-medium">{muted ? '음소거' : '소리'}</span>
                </button>
              </div>

              {/* Bottom info */}
              <div className="absolute bottom-0 left-0 right-0 p-4 pb-6 bg-gradient-to-t from-black/80 to-transparent z-10">
                <Link href={`/yt/channel/${short.channelId}`} className="flex items-center gap-2 mb-2">
                  <div className="w-8 h-8 bg-[#333] rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                    {short.channelTitle?.[0] || 'C'}
                  </div>
                  <span className="text-white text-sm font-semibold hover:underline">{short.channelTitle}</span>
                </Link>
                <p className="text-white text-sm line-clamp-2">{short.title}</p>
                {short.viewCount && (
                  <p className="text-gray-300 text-xs mt-1">조회수 {short.viewCount}회</p>
                )}
              </div>
            </div>
          </div>
